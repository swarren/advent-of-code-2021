...
func readParseInput(fn string) Diagram {
	...
	line, err = r.ReadString(10)
	check(err)
	parseSideRooms(diagram, line, 5)

	diagram[Coord{3, 3}] = Amphipod{'D', false}
	diagram[Coord{5, 3}] = Amphipod{'C', false}
	diagram[Coord{7, 3}] = Amphipod{'B', false}
	diagram[Coord{9, 3}] = Amphipod{'A', false}
	diagram[Coord{3, 4}] = Amphipod{'D', false}
	diagram[Coord{5, 4}] = Amphipod{'B', false}
	diagram[Coord{7, 4}] = Amphipod{'A', false}
	diagram[Coord{9, 4}] = Amphipod{'C', false}
...
func amphipodMovesMoved(diagram Diagram, startCoord Coord, amphipod Amphipod) []Move {
	...
	tgtCoord := Coord{homeX, 0}
	tgtFull := false
	for y := 5; y >= 2; y-- {
		// Home room occupied by a different type
		tgtCoord.y = y
		amphipodInRoomBot, tgtFull := diagram[tgtCoord]
		if tgtFull && amphipodInRoomBot.abcd != amphipod.abcd {
			return []Move{}
		}
		if !tgtFull {
			break
	...
	return []Move{{tgtCoord, cost + ((tgtCoord.y - 1) * moveCost)}}
}
