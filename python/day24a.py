...
for i in instructions:
    ...
    if fields[0] == "inp":
        ...
        if digits == 0:
            ...
            max = 9
            min = 1
        emit(f"for (int64_t digit{digits} = {max}; digit{digits} >= {min}; digit{digits}--) {{")
        indent()
...
int main(int argc, char *argv[]) {
    ...
    work.reserve(9*9*9);
    for (int64_t digit0v = 9; digit0v >= 1; digit0v--) {
        for (int64_t digit1v = 9; digit1v >= 1; digit1v--) {
            for (int64_t digit2v = 9; digit2v >= 1; digit2v--) {
