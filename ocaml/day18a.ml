...
let sn = List.fold_left snailnum_add (List.hd input) (List.tl input)
(* let () = print_endline (snailnum_printer sn) *)
let () = print_endline (string_of_int (snailnum_magnitude sn))
